...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagTemp_value)
    {
        info->event[info->num] = EVENT_Temp_value;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueTemp_value,(uint8_t *)&issuedData[byteFieldOffset],Temp_value_LEN);
        byteFieldOffset += Temp_value_LEN;
        dataPoints->valueTemp_value = gizX2Y(Temp_value_RATIO,  Temp_value_ADDITION, elongateP0FormTmp.devStatus.valueTemp_value); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagHumi_value)
    {
        info->event[info->num] = EVENT_Humi_value;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueHumi_value,(uint8_t *)&issuedData[byteFieldOffset],Humi_value_LEN);
        byteFieldOffset += Humi_value_LEN;
        dataPoints->valueHumi_value = gizX2Y(Humi_value_RATIO,  Humi_value_ADDITION, elongateP0FormTmp.devStatus.valueHumi_value); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagPM25_value)
    {
        info->event[info->num] = EVENT_PM25_value;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuePM25_value,(uint8_t *)&issuedData[byteFieldOffset],PM25_value_LEN);
        byteFieldOffset += PM25_value_LEN;
        dataPoints->valuePM25_value = gizX2Y(PM25_value_RATIO,  PM25_value_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valuePM25_value));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagSmog_value)
    {
        info->event[info->num] = EVENT_Smog_value;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueSmog_value,(uint8_t *)&issuedData[byteFieldOffset],Smog_value_LEN);
        byteFieldOffset += Smog_value_LEN;
        dataPoints->valueSmog_value = gizX2Y(Smog_value_RATIO,  Smog_value_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valueSmog_value));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueTemp_value != cur->valueTemp_value)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagTemp_value = 1;
        ret = 1;
    }
    if(last->valueHumi_value != cur->valueHumi_value)
    {
        GIZWITS_LOG("valueHumi_value Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagHumi_value = 1;
        ret = 1;
    }
    if(last->valuePM25_value != cur->valuePM25_value)
    {
        GIZWITS_LOG("valuePM25_value Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagPM25_value = 1;
        ret = 1;
    }
    if(last->valueSmog_value != cur->valueSmog_value)
    {
        GIZWITS_LOG("valueSmog_value Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagSmog_value = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagTemp_value)
    {
        devStatusTmp.valueTemp_value = gizY2X(Temp_value_RATIO,  Temp_value_ADDITION, dataPoints->valueTemp_value); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueTemp_value,Temp_value_LEN);
        byteFieldOffset += Temp_value_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagHumi_value)
    {
        devStatusTmp.valueHumi_value = gizY2X(Humi_value_RATIO,  Humi_value_ADDITION, dataPoints->valueHumi_value); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHumi_value,Humi_value_LEN);
        byteFieldOffset += Humi_value_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagPM25_value)
    {
        devStatusTmp.valuePM25_value = exchangeBytes(gizY2X(PM25_value_RATIO,  PM25_value_ADDITION, dataPoints->valuePM25_value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuePM25_value,PM25_value_LEN);
        byteFieldOffset += PM25_value_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSmog_value)
    {
        devStatusTmp.valueSmog_value = exchangeBytes(gizY2X(Smog_value_RATIO,  Smog_value_ADDITION, dataPoints->valueSmog_value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSmog_value,Smog_value_LEN);
        byteFieldOffset += Smog_value_LEN;
    }
