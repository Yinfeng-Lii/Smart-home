...
void Key_scan(void)
{
	if(KEY1) //如果没有按键按下
	{
		...
		if(key1_short_flag)
		{
			key1_short_flag = 0;	//清零长按标志位
			KeyNum = 1; //赋键值编码 长按键值编码
		}		
	}
	else if(!key1_lock_flag)
	{
		...
		{
			key1_cnt = 0; //清零计数标志
			key1_short_flag = 1; //赋键值编码 短按键值编码
	...
	if(KEY2) //如果没有按键按下
	{
		...
		if(key2_short_flag)
		{
			key2_short_flag = 0;	//清零长按标志位
			KeyNum = 2; //赋键值编码 长按键值编码
		}		
	}
	else if(!key2_lock_flag)
	{
		...
		{
			key2_cnt = 0; //清零计数标志
			key2_short_flag = 2; //赋键值编码 短按键值编码
	...
	if(KEY3) //如果没有按键按下
	{
		...
		if(key3_short_flag)
		{
			key3_short_flag = 0;	//清零长按标志位
			KeyNum = 3; //赋键值编码 长按键值编码
		}
		KeyHerdware();		
	}
	else if(!key3_lock_flag)
	{
		...
		{
			key3_cnt = 0; //清零计数标志
			key3_short_flag = 3; //赋键值编码 短按键值编码
	...
	if(KEY4) //如果没有按键按下
	{
		...
		if(key4_short_flag)
		{
			key4_short_flag = 0;	//清零长按标志位
			KeyNum = 4; //赋键值编码 长按键值编码
		}		
	}
	else if(!key4_lock_flag)
	{
		...
		{
			key4_cnt = 0; //清零计数标志
			key4_short_flag = 4; //赋键值编码 短按键值编码
